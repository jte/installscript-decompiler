...
BasicBlock* ControlFlowGraph::GetByLabel(std::string label)
{
    auto it = std::find_if(basicBlocks.begin(), basicBlocks.end(), [&label](BasicBlock* bb) { return bb->GetLabel() == label;  });
    return it != basicBlocks.end() ? *it : nullptr;
}

std::vector<BasicBlock*>::iterator ControlFlowGraph::GetByLabelIt(std::string label)
{
    auto it = std::find_if(basicBlocks.begin(), basicBlocks.end(), [&label](BasicBlock* bb) { return bb->GetLabel() == label;  });
    return it;
}