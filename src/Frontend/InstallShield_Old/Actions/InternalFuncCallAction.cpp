	...
	CActionWithArgs(script, filePtr)
{
	...
	size_t numArguments = m_script->GetFnById(m_eventIndex).prototype->GetArguments().size();
...
{
	auto proto = m_script->GetFnById(m_eventIndex).prototype;
	auto protoName = proto->GetName();
	if (protoName.empty())
	{
		os << "Func_" << proto->GetBBId();
	}
	else
	{
		os << protoName;
